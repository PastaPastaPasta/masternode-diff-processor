...
use crate::tests::json_from_core_snapshot::{
    masternode_list_from_genesis_diff, snapshot_to_snapshot, QRInfo,
};
...
pub fn test_from_snapshot() {
    let qrinfo: QRInfo = serde_json::from_slice(&message_from_file(
        "snapshot_0000021715c8575620382ceee42cc7556bac5ed395eaf9c75e2119aa2876a1e0.json"
            .to_string(),
    ))
    .unwrap();
    ...
    let mn_list_diff_tip =
        masternode_list_from_genesis_diff(qrinfo.mn_list_diff_tip, block_height_lookup);
    let mn_list_diff_h =
        masternode_list_from_genesis_diff(qrinfo.mn_list_diff_h, block_height_lookup);
    let mn_list_diff_h_c =
        masternode_list_from_genesis_diff(qrinfo.mn_list_diff_at_hminus_c, block_height_lookup);
    let mn_list_diff_h_2c =
        masternode_list_from_genesis_diff(qrinfo.mn_list_diff_at_hminus2c, block_height_lookup);
    let mn_list_diff_h_3c =
        masternode_list_from_genesis_diff(qrinfo.mn_list_diff_at_hminus3c, block_height_lookup);
    ...
    println!(
        "rotated_quorums at h ({}: {})",
        mn_list_diff_h.block_height, mn_list_diff_h.block_hash
    );
    let cached_blocks = &context.blocks;
    let get_height = |hash: UInt256| {
        cached_blocks
            .iter()
            .find(|block| block.hash == hash.clone().reversed())
            .unwrap()
            .height
    };
    ...
    if let Some(rotated_quorums_h) = mn_list_diff_h.added_quorums.get(&chain.isd_llmq_type()) {
        rotated_quorums_h
            .into_iter()
            .for_each(|(&llmq_block_hash, entry)| {
                println!(
                    "rotated_quorum: ({}: {})",
                    llmq_block_hash,
                    llmq_block_hash.clone().reversed()
                );
